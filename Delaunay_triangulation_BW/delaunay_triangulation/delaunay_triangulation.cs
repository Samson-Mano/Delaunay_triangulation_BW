...
namespace Delaunay_triangulation_BW.delaunay_triangulation
{
    public class delaunay_triangulation
    {
        ...
        public void create_super_triangle(List<point_store> p_inpt_points)
        {
            ...
            List<point_store> inpt_points = new List<point_store>();
            inpt_points = p_inpt_points.OrderBy(obj => obj.pt_coord.x).ThenBy(obj => obj.pt_coord.y).ToList();
        ...
        public int[] add_four_triangles(point_store new_pt, List<int> tri_corner_four_pts, List<int> tri_corner_four_edges)
        {
        ...
        public int[] add_three_triangles(point_store new_pt, List<int> tri_three_pts, List<int> tri_three_edges)
        {
        ...
        public int[] add_two_triangles(point_store c_pt, List<int> tri_corner_four_pts, List<int> tri_corner_four_edges)
        {
        ...
        {
            point_store[] x_sorted = all_input_vertices.OrderBy(obj => obj.pt_coord.x).ToArray();
            point_store[] y_sorted = all_input_vertices.OrderBy(obj => obj.pt_coord.y).ToArray();
            ...
            x_zero = (x_sorted[x_sorted.Length - 1].pt_coord.x + x_sorted[0].pt_coord.x) * 0.5;
            y_zero = (y_sorted[y_sorted.Length - 1].pt_coord.y + y_sorted[0].pt_coord.y) * 0.5;
            ...
            this.points_data.add_point(s_p2);
            this.points_data.add_point(s_p3);
        ...
        private point_d get_triangle_midpt(point_d pt1, point_d pt2, point_d pt3)
        {
        ...
        private double get_edge_length(point_d pt1, point_d pt2)
        {
            // get distance between two point
            return Math.Sqrt((pt1.x - pt2.x) * (pt1.x - pt2.x) + (pt1.y - pt2.y) * (pt1.y - pt2.y));
        }

        private double get_minimum_of_three(double d1, double d2, double d3)
        {
