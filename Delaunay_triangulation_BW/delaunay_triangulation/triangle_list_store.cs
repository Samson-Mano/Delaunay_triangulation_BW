...
namespace Delaunay_triangulation_BW.delaunay_triangulation
{
    public class triangle_list_store
    {
        ...
        public triangle_store get_bad_triangle(List<point_d> outter_bndry_pts, double Bv, double Hv)
        {
            ...
            {
                if (is_point_in_boundary(bad_triangle.mid_pt, outter_bndry_pts) == true)
                {
        ...
        private bool is_point_in_boundary(point_d i_pt,List<point_d> bndry_pts)
        {
            ...
            double t_angle = point_d.GetAngle(bndry_pts[bndry_pts.Count - 1], i_pt, bndry_pts[0]);
